...
class SegmentationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 64):
        ...
        self.feature1 = nn.Sequential(
            nn.Conv2d(input_channels, out_channels=self.hidden_layer_dim, kernel_size=1, stride=1, padding=0),
            nn.ReLU()
        )

        self.feature1[0].weight.data = nn.init.kaiming_normal_(self.feature1[0].weight.data, mode='fan_in', nonlinearity='relu')
        self.feature1[0].bias.data = torch.zeros(self.hidden_layer_dim)
        ...
        self.feature2[0].weight.data = nn.init.kaiming_normal_(self.feature2[0].weight.data, mode='fan_in', nonlinearity='relu')
        self.feature2[0].bias.data = torch.zeros(self.hidden_layer_dim)

        self.upsample = nn.functional.interpolate
        self.num_classes = num_classes

        self.conv_softmax = nn.Sequential(
            nn.Conv2d(in_channels=self.hidden_layer_dim, out_channels=self.num_classes+1, kernel_size=1, stride=1, padding=0),
            nn.ReLU(),
            nn.Softmax(dim=1)
        )

        self.conv_softmax[0].weight.data = nn.init.kaiming_normal_(self.conv_softmax[0].weight.data, mode='fan_in', nonlinearity='relu')
        self.conv_softmax[0].bias.data = torch.zeros(num_classes+1)
    ...
    def forward(self, feature1, feature2):
        ...
        feat1 = self.feature1(feature1)
        feat2= self.feature2(feature2)
        
        feat2_interp = self.upsample(feat2,size=(h,w),mode='bilinear')
        
        features = feat1 + feat2_interp
        
        features_upsampled = self.upsample(features ,size=(H,W),mode='bilinear')
        
        probability = self.conv_softmax(features_upsampled)
...
class TranslationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 128):
        ...
        self.conv=nn.Conv2d(in_channels=self.hidden_layer_dim, out_channels=self.num_classes, kernel_size=1, stride=1, padding=0)
           
        

        self.conv.weight.data = nn.init.kaiming_normal_(self.conv.weight.data, mode='fan_in', nonlinearity='relu')
        self.conv.bias.data = torch.zeros(self.num_classes)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, feature1, feature2):
        ...
        translation = self.conv(features_upsampled)
...
class RotationBranch(nn.Module):
    ...
    def __init__(self, feature_dim = 512, roi_shape = 7, hidden_dim = 4096, num_classes = 10):
        ...
        self.linear1=nn.Linear(feature_dim*self.roi_shape*self.roi_shape,out_features=self.hidden_layer_dim)
        self.linear1.weight.data = nn.init.kaiming_normal_(self.linear1.weight.data, mode='fan_in', nonlinearity='relu')
        self.linear1.bias.data = torch.zeros(self.hidden_layer_dim)
        self.relu1 = nn.ReLU()
        self.linear2=nn.Linear(self.hidden_layer_dim,out_features=4*num_classes)
        self.linear2.weight.data = nn.init.kaiming_normal_(self.linear2.weight.data, mode='fan_in', nonlinearity='relu')
        self.linear2.bias.data = torch.zeros(4*num_classes)
    ...
    def forward(self, feature1, feature2, bbx):
        ...
        quaternion= self.linear2(self.relu1(self.linear1(roi_feat_add_flat)))
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class PoseCNN(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            # Replace "pass" statement with your code
            imgs={}
            imgs['rgb']=input_dict['rgb']
            feature1, feature2 = self.feature_extractor(imgs)
            ...
            loss_dict["loss_centermap"] = loss_mean_abs_error(translation,input_dict['centermaps'])
            
            gt_bbx = gt_bbx.to('cuda')
            bbx_ft = IOUselection(bbx, gt_bbx.to('cuda'), self.iou_threshold)
            
            if bbx_ft.shape[0] >0:
                quaternion =  self.rotation_branch(feature1, feature2, gt_bbx[:,:5])                
                rot_gt = self.gtRotation(bbx_ft, input_dict)          
                rot_pred, label = self.estimateRotation(quaternion , bbx_ft)                 
                loss_dict["loss_R"] = loss_Rotation(rot_pred, rot_gt, label.long(),self.models_pcd)
            else:
                loss_dict["loss_R"] = 0
            ...
            with torch.no_grad():
                ...
                rot_pred, label = self.estimateRotation(quaternion, bbx)

                centres,depths = HoughVoting(segmentation, translation, num_classes=10)

                output_dict = self.generate_pose(rot_pred,centres,depths, bbx)
                ######################################################################
                #                            END OF YOUR CODE                        #
                ######################################################################
